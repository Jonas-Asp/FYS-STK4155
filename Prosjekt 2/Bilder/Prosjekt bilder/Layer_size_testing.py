...
class NeuralNetwork(object):
    ...
    # Do the forward propagation alogrithm
    def forward(self):
    ...
    # Do the backpropagaion algorithm
    ...
    # Do training with mse evaluations and mini batching
    ...
    # Evaluate mse error on test-set
    ...
    """ Change cost function and activation function here """
...
lmbd = [1, 1e-1, 1e-2]
eta  = [1e-3, 1e-2, 1e-1]
layers = [50, 75, 30, 85, 110]



"""  Check layer size for all lambda and eta values and plot """
fig, axes = plt.subplots(nrows=5, ncols=3, sharex='col',figsize=(30, 30))
count_it = 1
for i in range(len(layers)):    
    active_layers = layers[:i+1]
    title_layer = ','.join(str(e) for e in active_layers)
    

    for j in range(len(lmbd)):
        print("%.0f / %.0f" %(count_it, (len(layers)*len(lmbd)))) ; count_it += 1 
        
        NN = NeuralNetwork(X_train,Y_train, X_test, Y_test, active_layers, batch_size, eta[j], lmbd[j])
        mse_train, mse_test = NN.training(X_train,Y_train)
        minmse = np.min(mse_test)
        axes[i,j].plot(np.linspace(1,len(mse_train),len(mse_train)),mse_test, '--')
        axes[i,j].plot(np.linspace(1,len(mse_test),len(mse_test)),mse_test, '--', label=minmse)
        title = ("Lambda = %.5f, Eta = %.5f, Layer size(s) = " %(lmbd[j],eta[j])+title_layer)
        axes[i,j].set_title(title,y=1.04)
...
#NN = NeuralNetwork(X_train,Y_train, X_test, Y_test, (50,75,30)) # 3 first
#
#mse_train, mse_test = NN.training(X_train,Y_train)
#
#print(np.min(mse_train), "Train MSE")
#print(np.min(mse_test), "Test MSE")
#
#x1 = np.linspace(1,len(mse_train),len(mse_train))
#plt.plot(x1, mse_train, '--', label="Training")
#x2 = np.linspace(1,len(mse_test),len(mse_test))
#plt.plot(x2, mse_test, '--', label="Test")
#plt.legend()
#plt.show()
