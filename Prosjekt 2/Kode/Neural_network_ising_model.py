...
for i in range(len(eta)):
    count_it = 1
    Aktive_layers = layers[:0]
    for j in range(len(lmbd)):
        print("%.0f / %.0f" %(count_it, (len(eta)*len(lmbd)))) ; count_it += 1        
        NN = NeuralNetwork(X_train,Y_train, X_test, Y_test, (50,75,30), batch_size, eta[i], lmbd[j])
        ...
        title = ("Lambda = %.5f, Eta = %.5f" %(lmbd[j],eta[i]))
